...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/api/auth/login", {  // Ensure correct endpoint
      ...
      if (response.data.token && response.data.refreshToken) {
        login(response.data.token, response.data.refreshToken);  // Ensure the login function is correctly implemented in context
        navigate("/dashboard");
      } else {
